...
简单旅行在完整核心范围已经明确、确认或获得专项授权后，可以合并多个阶段；未满足强制视觉门条件时可直接生成完整候选。复杂旅行可以增加研究深度、专项内容和代表性视觉验证，但仍采用最小充分方案。
...
1. **旅行范围决定：** 选择目的地；目的地确定而完整核心体验范围尚未明确时，集中确认一次核心范围。
2. **总体行程方向决定：** 确认推荐的日程组合和主要取舍。
3. **视觉方向决定：** 仅在启用代表性视觉验证时出现。

“你决定”“按你推荐的来”或要求一次性完成的一般授权，可覆盖目的地推荐及普通后续决定，但不跳过核心范围确认，也不自动授权整体视觉方向。只有用户分别明确授权核心景点与体验范围、或整体视觉方向也由模型决定时，模型才在对应门禁中代为作出决定；用户沉默不构成授权。
...
- 目的地未知：进入入口 A。
- 目的地已知：进入入口 B。
- 用户已经给出完整必去与不去范围：可跳过核心范围确认，只检查现实可行性后进入研究与行程组合。
...
用户已经授权模型决定目的地时，模型选择首选并进入核心体验范围判断，不再为了形式要求用户确认目的地。

### 如何进入下一阶段

目的地确定后与入口 B 汇合；核心范围经用户确认、已完整给出或获得专项授权后，才进入研究与可执行行程组合。
...
在已知目的地内研究适合用户的景点、活动与城市体验，先形成有判断的核心范围建议，并在需要时真实停在一次集中确认门。
...
2. 只说明真正影响选择的因素：实际体验、适配性、大致用时、与其他地点的空间组合，以及预约／票务、客流、费用和体力代价。
3. 必要时说明候选是否值得增加一天或一晚、对酒店区域与行程骨架的影响，以及是否值得在最终视觉攻略中做知识深化；确认前不最终选择酒店或交通组合。
4. 将候选组织为：
   - 推荐进入核心行程；
   - 条件合适时可以安排；
   - 不建议纳入本次旅行，或用户已经排除。
5. 给出推荐的总体旅行方向，而不是要求用户自己拼接名单。
6. 用户尚未明确完整范围时，一次集中请其把主要候选标为“必去／可以安排／不去／仍需比较”，然后停止。

### 得到什么结果

确认门前是一份精炼的范围建议；确认或专项授权后，才形成包括核心体验、可选体验、排除项和总体方向的已收敛范围。

### 是否需要用户决定

只要目的地已确定而完整核心景点与体验范围尚未明确，就需要这一次集中确认。一般的“你决定／直接做完整方案”不能跳过；只有用户明确表示核心景点与体验范围也由模型决定、无需确认时，模型才采用推荐范围继续。

### 如何进入下一阶段

- 用户确认或明确作出核心范围专项授权后，进入研究与行程组合。
- 确认前不得最终确定实际旅行天数和住宿晚数、最终酒店、最终城际交通组合、详细逐日行程或完整视觉攻略。
- 用户已经给出完整必去与不去范围时，可跳过确认门，只检查现实可行性并继续。
...
内容已经足以支持现场使用后，进入视觉方向判断与生成。简单旅行可把本阶段并入行程组合；未满足强制代表性视觉条件时直接生成完整候选。
...
2. 判断是否同时满足代表性视觉验证的强制启用条件：多图成品共享整体视觉语言；用户提供重要视觉参考或把视觉完成度作为核心验收目标；错误方向会造成明显批量返工。
3. **不启用时：** 直接生成完整视觉候选。
4. **启用时：**
   - 先生成少量能够代表整套风险的实际图片，而不是只给风格说明、设计系统、页面目录或文字线框；
   - 代表页覆盖本次真正不同的高风险内容类型，数量不固定；任务同时含总览空间关系和复杂知识页时，通常都应覆盖，但不固化为两张模板；
   - 用户未明确授权整体视觉方向由模型决定时，展示实际代表图并等待确认；
   - 用户已作出该项明确授权时，由模型和唯一的独立 Reviewer 查看实际代表图及用户参考，方向成立后再继续；
...
仅在代表性视觉验证被启用且用户未明确授权整体视觉方向由模型决定时，需要用户确认方向。一般的“直接做完整方案”不构成这项授权。未同时满足强制条件时，不增加这一确认点。
...
2. Reviewer 使用整体专业判断，重点识别真正影响旅行使用的问题，例如：
   ...
   - 是否遗漏会实质影响执行的重要内容。
   - 成品是否报告化、简报化、Dashboard 化、Notion 卡片化、通用 PPT 化或机械复用卡片模板；
   - 目的地和当前景点能否一眼识别，图片是否真正解释路线、空间或知识，总览是否形成整趟旅行的空间骨架；
   - 是否有大面积无功能空白而正文仍过小或关键信息缺失、封面与内页割裂、页面必须逐段阅读才能恢复上下文，或明显低于用户明确认可的参考维度。
3. Reviewer 只报告具体问题、出现位置、实际影响，以及它属于局部问题还是整体方向问题；不发明新需求、不扩大旅行范围，不规定具体布局、配色、组件或修图技术，也不自行增加页面、景点或旅行内容。
4. Generator 根据具体问题修复实际候选。
5. 修复后，由独立 Reviewer 查看修改后的实际成品，确认原问题是否解决、是否引入明显新问题。
6. 每一轮都必须由真实问题触发；没有实质问题时立即停止，不为完成流程而继续 Review。
7. 若 Reviewer 判断整套视觉方向未成立，回退到代表性视觉阶段，只重做代表图；不在完整批次上继续填空白、加装饰或逐页打补丁，方向按相应确认或独立 Review 成立后再重新扩量。
...
常规 Review 和修复不需要用户逐轮确认。整体方向失败时按代表性视觉门的既有授权处理：未明确授权模型决定整体视觉方向的用户需要确认新代表图；已经明确授权的任务由模型和独立 Reviewer 完成该门禁。最终交付后，由用户决定是否接受成品，或只重新打开明确的局部范围。
...
|---|---|---|---|
| 目的地 | 入口 A | 会改变整个旅行体验、预算和后续研究范围 | 可以，一般授权即可 |
| 核心景点与体验范围 | 目的地确定而完整范围尚未明确时 | 会决定实际天数、住宿、交通、行程骨架和内容范围 | 可以，但必须明确授权该范围也由模型决定、无需确认 |
| 总体行程方向 | 可执行行程形成后，存在重大取舍时 | 会改变时间分配、体力、固定活动兼容和核心体验 | 可以 |
| 代表性视觉方向 | 强制视觉门触发时 | 错误方向会造成大规模返工 | 可以，但必须明确授权整体视觉方向也由模型决定，并由独立 Reviewer 查看实际代表图和参考 |
...
当用户说“你决定”“按你推荐的来”“直接做完整方案”或表达等价一般授权时，模型应：

1. 明确给出自己选择的目的地或普通行程方向，而不是隐含决定。
2. 简要说明最重要的假设和取舍，例如预算层级、节奏、公共交通优先或固定活动不可移动。
3. 在当前连续任务中，把该授权延续到普通后续决定，不在每个阶段重复询问。
4. 目的地确定后，如完整核心景点与体验范围仍未明确，仍给出范围建议并停在集中确认门；只有用户明确授权该范围也由模型决定、无需确认时才继续。
5. 强制代表性视觉门触发后，只有用户明确授权整体视觉方向也由模型决定时，才不等待用户亲自确认；仍须由独立 Reviewer 查看实际代表图和参考。
6. 在已通过上述门禁后，主动完成研究、行程、深化、内容拆分和视觉生成。只有新发现会推翻主要假设、明显增加预算、破坏固定活动或改变核心体验时，才再次请用户决定。
7. 不把用户沉默自动解释为任何授权。

推荐的表达方式是：

> 我将按首选方向继续，主要假设是……；接下来普通安排由我完成。核心景点与体验范围、以及强制触发的整体视觉方向，只有在你明确授权相应范围时才由我代为决定。
...
- 候选比较必须给出首选结论，不能只列选项。
- 用户未点名目的地候选，但继续要求下一步、要求完整方案或表达一般授权时，模型可按首选目的地继续并说明假设。
- 目的地确定后，完整核心景点与体验范围仍未明确且用户未作专项授权时，必须停在集中确认门；不以一般授权或沉默继续。
- 其他高影响决定确实无法代替时，停在对应关键点；不通过继续追问普通细节制造额外轮次。
...
|---|---|---|---|
| 简单周末游 | 完整核心范围明确后可合并需求理解、研究和行程组合；内容深化只处理一两个关键地点；未满足强制视觉门条件时直接生成完整候选 | 清楚的推荐结论、现实通勤、每日主线、手机可用成品、一次独立 Review | 独立事实包、专题研究文件、不必要的代表页门禁、多 Reviewer、多轮审批 |
...
| 博物馆／主题乐园／大型场馆主导 | 增加场馆专项研究和内部路线；满足强制触发条件时做代表性实际图片 | 入口、内部顺序、重点内容、时间分配、预约规则、现场易错点 | 自动增加多个 Reviewer、固定设计系统或庞大事实矩阵 |
...
同时满足以下条件时，必须启用代表性视觉验证：

- 最终会生成多张共享整体视觉语言的图片；
- 用户提供了重要参考成品，或明确把视觉完成度作为核心验收目标；
- 错误方向会造成明显批量返工。

启用后应遵循：

1. 先提炼用户明确认可的识别性、图文／地图／路线整体性、旅行叙事、信息密度、画布利用、手机扫读、图片解释作用和构图差异；这些维度必须进入代表图与 Reviewer 对比，不只停在文字说明。
2. 先生成少量有代表性的**实际图片**，而不是风格说明、设计系统、页面目录或文字线框。
3. 代表图覆盖本次真正不同的高风险内容类型，不固定数量；总览空间关系与复杂知识页都实际存在时，通常分别覆盖。
4. 方向未成立前，不批量扩散。
5. 用户未明确授权整体视觉方向由模型决定时，展示实际代表图并等待确认；一般授权不够。
6. 用户已明确作出该项授权时，由模型和唯一独立 Reviewer 查看实际代表图及用户参考，方向成立后再批量继续。
7. 未同时满足强制条件时，直接生成完整候选，不增加门禁。
...
- Reviewer 未参与当前候选的生成。
- 只保留这一个 Reviewer，不增加固定多 Reviewer 或固定角色编排。
...
- 判断问题属于局部缺陷还是整体视觉方向未成立；报告化／模板化、弱目的地识别、装饰性图片、机械复用构图、无功能空白伴随小字或信息缺失、总览缺少空间骨架、封面内页割裂、无法快速扫图恢复语境，以及明显低于用户认可参考维度，都可构成方向失败。
...
- 整体方向未成立：回退到代表性视觉阶段，只重做代表图；方向按既有授权完成确认或独立 Review 后再扩量，不在完整批次上继续打补丁。
...
7. 最终攻略中的“出发前更新事项”如何呈现，以及用户重新调用 Skill 更新这些事项时的最小交互方式。
