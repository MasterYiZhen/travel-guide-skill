...
当前候选为 **v0.1.0-rc.4**，推荐安装文件为 [当前 ZIP](../dist/upload/travel-guide-skill-v0.1.0-rc.4.zip)。实现与安装包检查结果见 [rc.4 检查记录](../evals/results-v0.1.0-rc.4.md)。

本版完成两处措辞及相关规则一致性检查、E04／E05／E07 的定向文本推演、结构和打包一致性检查。真实旅行任务验证尚未执行，待安装后通过完整旅行任务检查实际图片质量、系列一致性、独立审核修复和用户验收流程。文本推演、安装包完整性和云端加载是不同范围的检查，均不替代真实旅行任务结果。
...
Git 更新不会自动同步到已上传的云端 Skill，需要在技能管理界面更新。提交、远端 push、GitHub Release 和云端替换是分别执行的动作，应按当前授权范围进行。本版交付包括 Git 提交、安装 ZIP 和 GitHub 推送；推送结果以远端分支及交付回复中的提交号为准。创建 GitHub Release 和替换云端安装仍需分别执行。
...
| `dist/releases/` | 版本化安装归档，已有 ZIP 保留且不覆盖；rc.1—rc.3 仅供历史追溯或回退。 |
...
运行时变化时选择递增版本号；目标版本已存在则改用新版本，不重写历史 ZIP。仅修改开发文档、安装包内容未变时，不强制新增 ZIP。下一候选可使用 `v0.1.0-rc.5`，执行前检查是否被占用。
...
以下在仓库根目录执行，仅在运行时变化或确需重新发布时使用。示例采用下一候选版本；当前 rc.4 可直接安装。

```bash
VERSION=v0.1.0-rc.5 python3 - <<'PY'
...
- 按 [行为场景](../evals/evals.md) 记录所用方法。本版方法为受影响场景的定向文本推演，记录下一动作、等待节点、角色及结束条件；不计作真实图片或 Agent 执行。
