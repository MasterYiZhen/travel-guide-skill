...
当前候选为 **v0.1.0-rc.6**，推荐安装文件为 [当前 ZIP](../dist/upload/travel-guide-skill-v0.1.0-rc.6.zip)。实现与安装包检查结果见 [rc.6 检查记录](../evals/results-v0.1.0-rc.6.md)。

本版完成六项个人偏好的规则覆盖检查、E15 与 E14 扩展文本推演及 E01—E14 回归，并实际执行结构和打包一致性检查。真实旅行任务验证尚未执行，待安装后检查成品中的睡眠与出发、动线、补给及时间预算能否理解并执行，以及实际图片质量、系列一致性、独立审核修复和用户验收流程。规则与文本场景、实际工具执行、真实旅行任务效果分别记录；现场效果以实际使用反馈为依据。
...
| `dist/releases/` | 版本化安装归档，已有 ZIP 保留且不覆盖；rc.1—rc.5 仅供历史追溯或回退。 |
...
运行时变化时选择递增版本号；目标版本已存在则改用新版本，不重写历史 ZIP。仅修改开发文档、安装包内容未变时，不强制新增 ZIP。下一候选可使用 `v0.1.0-rc.7`，执行前检查是否被占用。
...
以下在仓库根目录执行，仅在运行时变化或确需重新发布时使用。示例采用下一候选版本；当前 rc.6 可直接安装。

```bash
VERSION=v0.1.0-rc.7 python3 - <<'PY'
...
- 按 [行为场景](../evals/evals.md) 记录所用方法。本版检查 E15、E14 扩展并回归 E01—E14，记录睡眠与出发、停留与净游览时间、路线及用餐休息取舍及下一动作、等待节点、角色和结束条件；不计作真实图片或 Agent 执行。
...
| 真实旅行任务效果 | 安装后选择含前夜活动、全天交通、大型景点及跨午餐参观的完整旅行任务，记录实际研究、制图、审核和用户验收结果；修复、复审或现场使用未发生时分别标为未验证。 |
