...
Git 更新不会自动同步到已上传的云端 Skill，需要在技能管理界面更新。提交、远端 push、GitHub Release 和云端替换是分别执行的动作，应按当前授权范围进行。本版实现提交 `42808b7` 及安装 ZIP 已推送到 GitHub，并已快进合并到 main；本次会话未创建 GitHub Release 或替换云端安装。
