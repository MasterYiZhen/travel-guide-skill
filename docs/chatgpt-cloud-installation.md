...
当前候选为 **v0.1.0-rc.7**，推荐安装文件为 [当前 ZIP](../dist/upload/travel-guide-skill-v0.1.0-rc.7.zip)。实现与安装包检查结果见 [rc.7 检查记录](../evals/results-v0.1.0-rc.7.md)。

本版完成首轮实物参考、逐页结果核对和规格早检的规则覆盖检查、E16—E18 与 E10 扩展文本推演及 E01—E15 回归，并实际执行结构和打包一致性检查。本版真实旅行任务验证尚未执行，待安装后检查首轮器形准确性、局部一致性、生成失败发现时点、候选及打包一致性、实际规格与返工情况，并记录独立审核修复和用户验收流程。规则与文本场景、实际工具执行、真实旅行任务效果分别记录；现场效果以实际使用反馈为依据。
...
| `dist/releases/` | 版本化安装归档，已有 ZIP 保留且不覆盖；rc.1—rc.6 仅供历史追溯或回退。 |
...
运行时变化时选择递增版本号；目标版本已存在则改用新版本，不重写历史 ZIP。仅修改开发文档、安装包内容未变时，不强制新增 ZIP。下一候选可使用 `v0.1.0-rc.8`，执行前检查是否被占用。
...
以下在仓库根目录执行，仅在运行时变化或确需重新发布时使用。示例采用下一候选版本；当前 rc.7 可直接安装。

```bash
VERSION=v0.1.0-rc.8 python3 - <<'PY'
...
- 按 [行为场景](../evals/evals.md) 记录所用方法。本版检查 E16—E18、E10 扩展并回归 E01—E15，记录参考使用时点、生成失败处理、候选版本、规格与清晰度判断、交付后自检及既有流程节点；不计作真实图片或 Agent 执行。
...
| 真实旅行任务效果 | 安装后选择含具名文物或建筑及多页制作的完整旅行任务，记录首轮器形与局部一致性、生成失败发现时点、候选及打包一致性、规格与返工情况，以及实际研究、制图、审核和用户验收结果；修复、复审或现场使用未发生时分别标为未验证。 |
